...
- `Slurm With Gpu <slurm_with_gpu>`_ -- Configure GPU support on the new nodes if they have
  GPUs.
- `Configure Nfs <../Storage/configure_nfs>`_ -- Mount shared storage on new nodes.
- `Setup Openldap <../Authentication/setup_openldap>`_ -- Ensure LDAP clients are
