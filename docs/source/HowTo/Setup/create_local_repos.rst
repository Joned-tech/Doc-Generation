...
- The `Prepare Oim <prepare_oim>`_ procedure is complete (Pulp is running).
- The `Configure Inputs <configure_inputs>`_ procedure is complete (``software_config.json``
...
- `Build Cluster Images <build_cluster_images>`_ -- Build OS boot images using the local repos.
- `Discover Nodes <discover_nodes>`_ -- Discover and PXE-boot target nodes.
