...
- The `Prepare Oim <prepare_oim>`_ procedure is complete (OpenCHAMI and DHCP are
  running).
- The `Build Cluster Images <build_cluster_images>`_ procedure is complete (boot images are in
  MinIO).
- The `Create Mapping File <create_mapping_file>`_ procedure is complete.
- The `Configure Credentials <configure_credentials>`_ procedure is complete (BMC credentials
  configured).
...
- `Pxe Boot Nodes <pxe_boot_nodes>`_ -- Ensure all nodes complete provisioning.
- `Verify Cluster <verify_cluster>`_ -- Verify the cluster is operational.
- `Setup Slurm <../Slurm/setup_slurm>`_ -- Deploy Slurm on discovered nodes.
