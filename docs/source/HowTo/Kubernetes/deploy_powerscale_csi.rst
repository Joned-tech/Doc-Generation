...
- A Kubernetes service cluster is deployed (see `Setup Service K8S <setup_service_k8s>`_).
...
- `Setup Telemetry <../Telemetry/setup_telemetry>`_ -- Deploy telemetry with
  PowerScale-backed persistent storage.
- `Configure Nfs <../Storage/configure_nfs>`_ -- Configure NFS for Slurm compute
