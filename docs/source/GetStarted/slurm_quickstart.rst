...
.. note::
   ...
   `Prerequisites Checklist <prerequisites_checklist>`_. If you have not, stop here and finish
   that first.
...
.. warning::
   ...
   sufficient disk space (see `Prerequisites Checklist <prerequisites_checklist>`_) and a stable
   internet connection.
...
**Enable telemetry and monitoring**
   Follow `Full Deployment <full_deployment>`_ (Path B) starting from the telemetry
...
**Configure authentication (LDAP/FreeIPA)**
   ...
   ``auth.yml``. See the `Full Deployment <full_deployment>`_ guide for details.
...
.. note::
   ...
   - `Index <../Overview/index>`_ -- Architecture and component overview
   - `Full Deployment <full_deployment>`_ -- Add K8s and telemetry to this cluster
   - `Prerequisites Checklist <prerequisites_checklist>`_ -- Return to the master checklist
